<?php
// app/views/dashboard/groups/create.php
...
<div class="container mx-auto py-10 px-6">
    <div class="flex items-center mb-8">
        <a href="/dashboard/grupos" class="text-indigo-600 hover:text-indigo-800 mr-4">
            <i class="fas fa-arrow-left fa-lg"></i>
        </a>
        <h1 class="text-4xl font-bold text-gray-800">Crear Nuevo Grupo</h1>
    </div>

    <div class="bg-white p-8 rounded-lg card-shadow max-w-3xl mx-auto">
        <form action="/dashboard/grupos/process-create" method="POST" class="space-y-6">
            <fieldset class="border p-4 rounded-md">
                ...
                <div>
                    ...
                    <input type="text" name="nombre_grupo" id="nombre_grupo" required class="input-style" placeholder="Ej: 5to Grado - Grupo A">
                </div>
                <div class="mt-4">
                ...
                <div class="mt-4">
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3" class="input-style" placeholder="Notas sobre el grupo, horario, etc."></textarea>
                </div>
            </fieldset>
                        ...
                        <?php foreach ($estudiantes_disponibles as $estudiante): ?>
                            <div class="flex flex-col border-b pb-3 mb-3 last:border-b-0 last:pb-0">
                                <label class="inline-flex items-center mb-2">
                                    <input type="checkbox" name="estudiantes[]" value="<?= htmlspecialchars($estudiante['id']) ?>" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-200 inclusion-checkbox">
                                ...
                                <div id="observaciones_container_<?= htmlspecialchars($estudiante['id']) ?>" class="ml-6 mt-1 hidden">
                                    ...
                                    <textarea name="observaciones_inclusion_<?= htmlspecialchars($estudiante['id']) ?>" id="observaciones_inclusion_<?= htmlspecialchars($estudiante['id']) ?>" rows="2" class="input-style text-sm" placeholder="Ej: Requiere apoyo visual, TDAH, etc."></textarea>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
