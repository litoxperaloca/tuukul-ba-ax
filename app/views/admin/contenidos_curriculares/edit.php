<?php
// app/views/admin/contenidos_curriculares/edit.php
...
<div class="container mx-auto py-10 px-6">
    <div class="flex items-center mb-8">
        ...
        <h1 class="text-4xl font-bold text-gray-800">Editar Contenido Curricular: <?= htmlspecialchars($contenido['nombre_contenido']) ?></h1>
    </div>

    <div class="bg-white p-8 rounded-lg card-shadow max-w-xl mx-auto">
        <form action="/admin/contenidos-curriculares/process-update/<?= htmlspecialchars($contenido['id']) ?>" method="POST" class="space-y-6">
            <div>
                ...
                <select name="id_curso" id="id_curso" required class="input-style">
                    ...
                    <?php foreach ($cursos as $curso): ?>
                        <option value="<?= htmlspecialchars($curso['id']) ?>" <?= ($curso['id'] == $contenido['id_curso']) ? 'selected' : '' ?>>
            ...
            <div class="mt-4">
                ...
                <select name="tipo" id="tipo" required class="input-style">
                    <option value="contenido" <?= ($contenido['tipo'] === 'contenido') ? 'selected' : '' ?>>Contenido Curricular</option>
                    <option value="eje_articulador" <?= ($contenido['tipo'] === 'eje_articulador') ? 'selected' : '' ?>>Eje Articulador</option>
                </select>
            </div>
            <div class="mt-4">
                <label for="nombre_contenido" class="label-style">Nombre del Contenido / Eje</label>
                <input type="text" name="nombre_contenido" id="nombre_contenido" value="<?= htmlspecialchars($contenido['nombre_contenido']) ?>" required class="input-style" placeholder="Ej: Capacidades y habilidades motrices">
            </div>
            <div class="mt-4" id="pda_field_container">
                ...
                <textarea name="pda_descripcion" id="pda_descripcion" rows="3" class="input-style" placeholder="Descripción del PDA"><?= htmlspecialchars($contenido['pda_descripcion'] ?? '') ?></textarea>
            </div>
            <div class="mt-4">
                <label for="orden" class="label-style">Orden de Visualización (Opcional)</label>
                <input type="number" name="orden" id="orden" value="<?= htmlspecialchars($contenido['orden']) ?>" class="input-style" min="0">
            </div>
            <div class="flex items-center mt-4">
                <input type="checkbox" name="activo" id="activo" <?= $contenido['activo'] ? 'checked' : '' ?> class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function togglePdaField() {
            if (tipoSelect.value === 'contenido') {
                ...
                // No limpiar el valor al ocultar en edición para no perderlo si el usuario cambia de opinión
            }
        }
